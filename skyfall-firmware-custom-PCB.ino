...
// Custom SPI pins (adjust these as needed for your PCB)
...
const int requiredFallSamples = 1; // For fall detection (0.1s / updateInterval)
const float FALL_LOWER_THRESHOLD = 7.0;
const float FALL_UPPER_THRESHOLD = 11.0;

// ===== Variables for LSM6DSL sensor data =====
...
// ===== Web Server & WebSocket Handlers (same as original) =====
void setupWiFi()
{
...
void setup()
{
  ...
  spi2.begin(SCL_PIN, SDI_PIN, SDO_PIN, CS_PIN); // Initialize SPI with SCK, MISO, MOSI, and CS
  ...
  // Instead of mpu.begin(), initialize the LSM6DSL over SPI
  if (!initializeLSM6DSL())
  {
...
void loop()
{
  ...
  {
    mortarServo.write(115); // Deploy parachute (servo move)
