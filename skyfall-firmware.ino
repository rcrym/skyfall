...
// Storage for up to 200 IMU samples
...
// Step size (learning rate) and iterations
const float LEARNING_RATE = 0.01;
const int MAX_ITERS = 100;

// Calibration loss tracking
float rx = 0.0, ry = 0.0, rz = 0.0;  // Current offset estimates
float calibrationLoss = 0.0;  // Current calibration loss value
...
void webSocketEvent(uint8_t num, WStype_t type, uint8_t *payload, size_t length) {
  if (type == WStype_CONNECTED) {
    ...
    if (message == "resetFall") {
      ...
      // Added recalibration option
      sample_count = 0;
      calibrationDone = false;
      rx = ry = rz = 0.0; // reset them
      calibrateSensors();
...
void setup() {
  ...
  if (file) {
    file.println("Time,AX,AY,AZ,|A|,GX,GY,GZ,CalibrationLoss,FallDetected");  // Updated CSV header
    file.close();
  ...
  mpu.setFilterBandwidth(MPU6050_BAND_94_HZ);  // or even 184_HZ
...
void calibrateSensors() {
  ...
  for (int i = 0; i < calibrationSamples; i++) {
    ...
    // Collect gyro data for offset estimation
  ...
  // Estimate offsets using collected data
  if (sample_count > 10) {
...
// Gradient descent to find best rx, ry, rz
...
// For fall detection based on sustained acceleration
int sustainedFallCounter = 0;
const int requiredFallSamples = 1;  // 0.1s / 0.01s per update = 10 samples
...
void sendSensorData(unsigned long currentMillis) {
  ...
  // Run offset estimation every 500ms if enough samples have been collected
